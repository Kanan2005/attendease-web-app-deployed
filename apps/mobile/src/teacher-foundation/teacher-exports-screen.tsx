...
const EXPORT_TYPES: Array<{
...
}> = [
  {
    ...
    description: "Export a single lecture's attendance list with student names, roll numbers, and status.",
  ...
  },
  {
    ...
    description: "Per-student attendance percentages across all sessions. Useful for identifying at-risk students.",
  ...
  },
  {
    ...
    icon: "grid-outline",
    title: "Full Register",
    description: "Complete attendance matrix — every student × every session. Ideal for university submission.",
...
export function TeacherExportsScreen() {
  ...
  const activeExportType = EXPORT_TYPES.find((t) => t.key === selectedType)!
  ...
  useEffect(() => {
    if (!activeExportType.formats.includes(selectedFormat)) {
      setSelectedFormat(activeExportType.formats[0]!)
    }
  }, [selectedType, activeExportType, selectedFormat])

  const selectedClassroomLabel = (() => {
    if (isSessionExport) {
      return classroomsWithSessions.find((cr) => cr.id === selectedClassroomId)?.title ?? "Select Classroom"
    }
    return classrooms.find((cr) => cr.id === selectedClassroomId)?.displayTitle ?? "All Classrooms"
  })()

  const canSubmit = !createExportJobMutation.isPending && (isSessionExport ? Boolean(selectedSessionId) : true)
    ...
    try {
      await createExportJobMutation.mutateAsync(
        isSessionExport
          ...
          : { jobType: selectedFormat, filters: selectedClassroomId ? { classroomId: selectedClassroomId } : {} },
      )
    } catch {
  ...
  return (
    ...
    >
      ...
      <View style={[es.header, { paddingTop: insets.top + 8 }]}>
        ...
        <Text style={{ fontSize: 13, color: c.textMuted }}>Download attendance data for records or submission</Text>
      </View>
      ...
      <View style={es.typeSection}>
        {EXPORT_TYPES.map((type) => {
          ...
          return (
            <Pressable key={type.key} onPress={() => { setSelectedType(type.key); setClassroomDropdownOpen(false) }}
              style={[es.typeCard, {
              ...
              <View style={{ flex: 1, gap: 2 }}>
                <Text style={{ fontSize: 14, fontWeight: "700", color: active ? c.primary : c.text }}>{type.title}</Text>
                <Text style={{ fontSize: 11, color: c.textMuted }} numberOfLines={2}>{type.description}</Text>
              </View>
        ...
        <View style={es.section}>
          ...
          <View style={{ flexDirection: "row", gap: 8 }}>
            {activeExportType.formats.map((fmt) => {
              ...
              const icon: keyof typeof Ionicons.glyphMap = fmt.includes("PDF") ? "document-outline" : "grid-outline"
              return (
          ...
          <>
            ...
            {sessionsQuery.isLoading ? (
              ...
              <View style={{ alignItems: "center", paddingVertical: 16 }}>
                ...
                <Text style={{ fontSize: 13, color: c.textMuted, marginTop: 6 }}>No completed sessions yet</Text>
              </View>
            ) : (
              <>
                <Pressable onPress={() => setClassroomDropdownOpen((v) => !v)}
                  style={[es.dropdownTrigger, { borderColor: classroomDropdownOpen ? c.primary : c.border, backgroundColor: c.surfaceRaised }]}>
                  ...
                  <Ionicons name={classroomDropdownOpen ? "chevron-up" : "chevron-down"} size={16} color={c.textSubtle} />
                </Pressable>
                {classroomDropdownOpen ? (
                  <View style={[es.dropdownMenu, { borderColor: c.border, backgroundColor: c.surfaceRaised }]}>
                    {classroomsWithSessions.map((cr) => {
                      const active = selectedClassroomId === cr.id
                      return (
                        <Pressable key={cr.id} onPress={() => { setSelectedClassroomId(cr.id); setClassroomDropdownOpen(false); setSelectedSessionId(""); setShowAllSessions(false) }}
                          style={[es.dropdownItem, active && { backgroundColor: c.primarySoft }]}>
                          <Text style={{ fontSize: 13, color: active ? c.primary : c.text, fontWeight: active ? "700" : "400", flex: 1 }} numberOfLines={1}>{cr.title}</Text>
                ...
                <View style={{ flexDirection: "row", alignItems: "center", justifyContent: "space-between", marginTop: 12 }}>
                  ...
                  {filteredSessions.length > 0 ? (
                    <Text style={{ fontSize: 12, color: c.textMuted }}>{filteredSessions.length} total</Text>
                  ) : null}
                </View>
                {filteredSessions.length === 0 ? (
                  <Text style={{ fontSize: 12, color: c.textMuted }}>No sessions in this classroom</Text>
                ) : (
                  <>
                    {(showAllSessions ? filteredSessions : filteredSessions.slice(0, 5)).map((s, i) => {
                      ...
                      const timeLabel = s.startedAt
                        ...
                        : s.lectureDate
                          ...
                          : fmtEnum(s.status)
                      return (
                        <Animated.View key={s.id} entering={FadeInDown.duration(150).delay(i * 20)}>
                          <Pressable onPress={() => setSelectedSessionId(s.id)}
                            style={[es.sourceCard, { backgroundColor: active ? c.primarySoft : c.surfaceRaised, borderColor: active ? c.primary : c.border }]}>
                            <View style={[es.sessionBadge, { backgroundColor: active ? c.primary : c.surfaceTint }]}>
                              <Text style={{ fontSize: 11, fontWeight: "800", color: active ? "#fff" : c.textSubtle }}>
                            ...
                            <View style={{ flex: 1, gap: 2 }}>
                              ...
                              <Text style={{ fontSize: 11, color: c.textMuted }}>
                                {timeLabel}{" · "}{s.presentCount}P / {s.absentCount}A
                              </Text>
                            </View>
                            {active ? <Ionicons name="checkmark-circle" size={20} color={c.primary} /> : <Ionicons name="ellipse-outline" size={20} color={c.border} />}
                          </Pressable>
                        </Animated.View>
                      )
                    })}
          ...
          <>
            ...
            {classroomsQuery.isLoading ? (
              ...
              <View style={{ alignItems: "center", paddingVertical: 16 }}>
                ...
                <Text style={{ fontSize: 13, color: c.textMuted, marginTop: 6 }}>No classrooms</Text>
              </View>
            ) : (
              <>
                <Pressable onPress={() => setClassroomDropdownOpen((v) => !v)}
                ...
                {classroomDropdownOpen ? (
                  <View style={[es.dropdownMenu, { borderColor: c.border, backgroundColor: c.surfaceRaised }]}>
                    <Pressable onPress={() => { setSelectedClassroomId(""); setClassroomDropdownOpen(false) }}
                      style={[es.dropdownItem, !selectedClassroomId && { backgroundColor: c.primarySoft }]}>
                      <Text style={{ fontSize: 13, color: !selectedClassroomId ? c.primary : c.text, fontWeight: !selectedClassroomId ? "700" : "400" }}>All Classrooms</Text>
                      {!selectedClassroomId ? <Ionicons name="checkmark" size={16} color={c.primary} /> : null}
                    </Pressable>
                    {classrooms.map((cr) => {
                      const active = selectedClassroomId === cr.id
                      return (
                        <Pressable key={cr.id} onPress={() => { setSelectedClassroomId(cr.id); setClassroomDropdownOpen(false) }}
                          style={[es.dropdownItem, active && { backgroundColor: c.primarySoft }]}>
                          <Text style={{ fontSize: 13, color: active ? c.primary : c.text, fontWeight: active ? "700" : "400", flex: 1 }} numberOfLines={1}>{cr.displayTitle}</Text>
                          {active ? <Ionicons name="checkmark" size={16} color={c.primary} /> : null}
                        </Pressable>
                      )
                    })}
      ...
      <View style={{ paddingHorizontal: 16, marginTop: 16 }}>
        {createExportJobMutation.error ? (
          <View style={{ flexDirection: "row", alignItems: "center", gap: 8, backgroundColor: c.dangerSoft, borderRadius: 10, padding: 10, marginBottom: 10 }}>
            <Ionicons name="alert-circle" size={16} color={c.danger} />
            <Text style={{ fontSize: 12, color: c.danger, flex: 1 }}>{mapTeacherApiErrorToMessage(createExportJobMutation.error)}</Text>
          </View>
        ) : null}
        <Pressable
        ...
        >
          <View style={{ flexDirection: "row", alignItems: "center", gap: 8 }}>
            ...
            <Text style={styles.primaryButtonLabel}>
              {createExportJobMutation.isPending ? "Requesting..." : `Export ${activeExportType.title}`}
            </Text>
          </View>
        </Pressable>
      </View>
      ...
      <View style={es.section}>
        ...
        {exportJobsQuery.isLoading ? (
          ...
          <Text style={{ fontSize: 12, color: c.danger }}>{mapTeacherApiErrorToMessage(exportJobsQuery.error)}</Text>
          ...
          <View style={{ alignItems: "center", paddingVertical: 24, gap: 6 }}>
            ...
            <Text style={{ fontSize: 11, color: c.textMuted, textAlign: "center", paddingHorizontal: 24 }}>
          ...
          exportJobsQuery.data.map((job, i) => {
            ...
            return (
              <Animated.View key={job.id} entering={FadeInDown.duration(200).delay(i * 30)}>
                <View style={[es.jobCard, { backgroundColor: c.surfaceRaised, borderColor: c.border }]}>
                  <View style={{ flexDirection: "row", alignItems: "center", gap: 10 }}>
                    <View style={[es.jobIcon, { backgroundColor: isReady ? c.successSoft : isFailed ? c.dangerSoft : c.surfaceTint }]}>
                      <Ionicons
                        name={isReady ? "checkmark-circle" : isFailed ? "close-circle" : "hourglass-outline"}
                        size={16} color={isReady ? c.success : isFailed ? c.danger : c.textSubtle}
                      />
                    </View>
                    <View style={{ flex: 1, gap: 1 }}>
                      <Text style={{ fontSize: 13, fontWeight: "700", color: c.text }}>{friendlyJobType(job.jobType)}</Text>
                      <Text style={{ fontSize: 11, color: c.textMuted }}>
                        {job.courseOfferingDisplayTitle ?? "All classrooms"} · {formatDateTime(job.requestedAt)}
                      </Text>
                    </View>
                    <View style={[es.statusBadge, {
                      backgroundColor: isReady ? c.successSoft : isFailed ? c.dangerSoft : c.surfaceTint,
                    }]}>
                      <Text style={{ fontSize: 10, fontWeight: "700", color: isReady ? c.success : isFailed ? c.danger : c.textSubtle }}>
                  ...
                  {job.latestReadyDownloadUrl ? (
                    <Pressable
                      onPress={() => { void Linking.openURL(job.latestReadyDownloadUrl ?? "") }}
                      style={[es.downloadBtn, { borderColor: c.primary }]}
                    >
                      <Ionicons name="cloud-download-outline" size={14} color={c.primary} />
                      <Text style={{ fontSize: 12, fontWeight: "600", color: c.primary }}>Download</Text>
                    </Pressable>
                  ) : null}
...
const es = StyleSheet.create({
  ...
  sourceCard: { flexDirection: "row", alignItems: "center", gap: 10, borderWidth: 1, borderRadius: 12, padding: 12 },
  sessionBadge: {
    width: 30, height: 30, borderRadius: 8, alignItems: "center", justifyContent: "center",
  },
  jobCard: { borderWidth: 1, borderRadius: 12, padding: 14, gap: 8 },
  jobIcon: { width: 32, height: 32, borderRadius: 16, alignItems: "center", justifyContent: "center" },
  statusBadge: { paddingHorizontal: 8, paddingVertical: 3, borderRadius: 6 },
  downloadBtn: { flexDirection: "row", alignItems: "center", gap: 6, alignSelf: "flex-start", paddingHorizontal: 12, paddingVertical: 6, borderRadius: 8, borderWidth: 1 },
})
