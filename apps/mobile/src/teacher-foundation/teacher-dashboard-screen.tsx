  ...
  if (dashboardError) {
    return (
      <View style={{ flex: 1, backgroundColor: c.surface, alignItems: "center", justifyContent: "center", padding: 32 }}>
        ...
        <Text style={{ fontSize: 14, color: c.danger, marginTop: 12, textAlign: "center" }}>{mapTeacherApiErrorToMessage(dashboardError)}</Text>
      </View>
    )
  }
  ...
  return (
    ...
    >
      ...
      <View style={[ds.header, { paddingTop: insets.top + 8 }]}>
        ...
        <Text style={{ fontSize: 13, color: c.textMuted }}>Classrooms, attendance, and sessions at a glance</Text>
      </View>

      {/* ── Spotlight ── */}
      <Animated.View entering={FadeInDown.duration(250).delay(50)}>
        <View style={[ds.spotlightCard, { backgroundColor: c.primarySoft, borderColor: c.primary }]}>
          <Text style={{ fontSize: 15, fontWeight: "700", color: c.primary }}>{dashboard.model.spotlight.title}</Text>
          <Text style={{ fontSize: 13, color: c.text }}>{dashboard.model.spotlight.message}</Text>
          <View style={{ flexDirection: "row", gap: 10, marginTop: 4 }}>
            <Link href={resolveTeacherDashboardActionHref(dashboard.model.spotlight.primaryAction, firstClassroomContext)} asChild>
              <Pressable style={[ds.spotlightBtn, { backgroundColor: c.primary }]}>
                <Text style={{ fontSize: 13, fontWeight: "700", color: c.primaryContrast }}>{dashboard.model.spotlight.primaryAction.label}</Text>
              </Pressable>
            </Link>
            {dashboard.model.spotlight.secondaryAction ? (
              <Link href={resolveTeacherDashboardActionHref(dashboard.model.spotlight.secondaryAction, firstClassroomContext)} asChild>
                <Pressable style={[ds.spotlightBtn, { backgroundColor: c.surfaceRaised, borderWidth: 1, borderColor: c.border }]}>
                  <Text style={{ fontSize: 13, fontWeight: "600", color: c.text }}>{dashboard.model.spotlight.secondaryAction.label}</Text>
                </Pressable>
              </Link>
            ) : null}
          </View>
      ...
      <View style={ds.section}>
        ...
        {dashboard.model.classroomHighlights.length ? (
          dashboard.model.classroomHighlights.map((classroom, i) => {
            ...
            return (
              <Animated.View key={classroom.classroomId} entering={FadeInDown.duration(200).delay(i * 30)}>
                <Link href={ctx.detail} asChild>
                  <Pressable style={[ds.card, { backgroundColor: c.surfaceRaised, borderColor: c.border }]}>
                    <View style={{ flexDirection: "row", alignItems: "center", gap: 10 }}>
                      ...
                      <View style={{ flex: 1, gap: 2 }}>
                        <Text style={{ fontSize: 14, fontWeight: "700", color: c.text }} numberOfLines={1}>{classroom.title}</Text>
                        <Text style={{ fontSize: 11, color: c.textMuted }} numberOfLines={1}>{classroom.supportingText}</Text>
                      </View>
                    ...
                    <View style={{ flexDirection: "row", gap: 12, marginLeft: 44 }}>
                      <View style={{ flexDirection: "row", alignItems: "center", gap: 4 }}>
                        ...
                        <Text style={{ fontSize: 11, color: c.textMuted }}>{classroom.sessionStateLabel}</Text>
                      </View>
                      <View style={{ flexDirection: "row", alignItems: "center", gap: 4 }}>
                        <Ionicons name="key-outline" size={12} color={c.textSubtle} />
                        <Text style={{ fontSize: 11, color: c.textMuted }}>{classroom.joinCodeLabel}</Text>
                      </View>
                    </View>
                  </Pressable>
                </Link>
          ...
          <View style={{ alignItems: "center", paddingVertical: 24 }}>
            ...
            <Text style={{ fontSize: 13, color: c.textMuted, marginTop: 6 }}>No classrooms yet</Text>
          </View>
        )}
      </View>
            ...
            return (
              <Animated.View key={item.id} entering={FadeInDown.duration(200).delay(i * 25)}>
                <Link href={teacherRoutes.sessionDetail(item.id)} asChild>
                  <Pressable style={[ds.card, { backgroundColor: c.surfaceRaised, borderColor: c.border }]}>
                    <View style={{ flexDirection: "row", alignItems: "center", gap: 10 }}>
                      <View style={[ds.iconCircle, { backgroundColor: isLive ? c.successSoft : c.surfaceTint }]}>
                        <Ionicons name={isLive ? "radio" : "timer-outline"} size={16} color={isLive ? c.success : c.textSubtle} />
                      </View>
                      <View style={{ flex: 1, gap: 2 }}>
                        <Text style={{ fontSize: 14, fontWeight: "700", color: c.text }} numberOfLines={1}>{item.classroomTitle}</Text>
                        <Text style={{ fontSize: 11, color: c.textMuted }} numberOfLines={1}>
                          {item.title} · {formatEnum(item.mode)} · {formatEnum(item.status)}
                        </Text>
                        <Text style={{ fontSize: 11, color: c.textMuted }}>
                          {item.presentCount} present / {item.absentCount} absent · {formatDateTime(item.timestamp)}
                        </Text>
                      </View>
                      <Ionicons name="chevron-forward" size={16} color={c.textSubtle} />
                    </View>
      ...
      <View style={[ds.section, { marginTop: 24 }]}>
        <Pressable onPress={signOut}
          style={{ flexDirection: "row", alignItems: "center", justifyContent: "center", gap: 8, borderRadius: 12, borderWidth: 1, borderColor: c.dangerBorder, backgroundColor: c.dangerSoft, paddingVertical: 14 }}>
