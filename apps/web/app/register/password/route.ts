import { AuthApiClientError, createAuthApiClient } from "@attendease/auth"
...
export async function POST(request: Request) {
  ...
  const failureUrl = new URL(
    resolveWebAuthEntryPath({ scope: "teacher", mode: "register", next }),
  ...
  try {
    const authSession = await authClient.registerTeacherAccount({
      displayName,
      email,
      password,
      platform: "WEB",
    ...
    console.error("Teacher web registration failed.", {
      email,
      error,
    })
    failureUrl.searchParams.set(
      "error",
      error instanceof AuthApiClientError && error.status === 409
        ? "email-in-use"
        : "invalid-credentials",
    )
    return NextResponse.redirect(failureUrl, 303)
